	...
	<main id="content">
        ...
        <div class="row" style="justify-content: right;">
        	...
        	<div class="row" style="padding:5px">
			 <div class="col-md-10">전체<span> ${paging.total}</span>/ 검색 <span> ${paging.cntPerPage}</span></div>
			 
			<div class="col-md-2">
			<form class="form" id="kboard-sort-form-1" method="get" action="#" style="text-align: end">
				...
				<select class="custom-select custom-select-sm" name="kboard_list_sort" onchange="jQuery('#kboard-sort-form-1').submit();" style="width:auto">
        	...
        	<div class="row">
        		...
        		<table class="table" style="text-align: center;">
        			<thead>
        				<tr>
        					<th text-align:center" width="70px">번호</th>
        					<th text-align:center">제목</th>
        					<th text-align:center" width="130px">작성자</th>
        					<th text-align:center" width="130px">작성일</th>
        					<th text-align:center" width="130px">조회</th>
        				</tr>
        			</thead>
        			<tbody>
        			<c:forEach items="${viewAll}" var="list">
        				<tr>
	        				<td>${list.pb_num}</td>
	        				<td>
	        				<a href="PBcontent?pb_num=${list.pb_num}">${list.pb_title}</a>
	        				</td>
	        				<td>${list.mem_num}</td>
	        				<td>${list.pb_rdate}</td>
	        				<td>${list.pb_view}</td>
			...
			<div class="container" style="padding:10px">
			  		...
			  		<c:if test="${paging.nowPage != 1 }">
						<li class="page-item"><a class="page-link" href="PBoard?nowPage=${paging.nowPage - 1}&cntPerPage=${paging.cntPerPage}">Previous</a></li>
					</c:if>	
			  		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p">
						<c:choose>
							<c:when test="${p == paging.nowPage }">
			    				<li class="page-item"><a class="page-link" aria-pressed="true"><b>${p}</b></a></li>
			    			</c:when>
			    			<c:when test="${p != paging.nowPage }">
			    				<li class="page-item"><a class="page-link" href="PBoard?nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a></li>
			    			</c:when>
						</c:choose>
					</c:forEach>
					<c:if test="${paging.nowPage != paging.lastPage}">
			    		<li class="page-item"><a class="page-link" href="PBoard?nowPage=${paging.nowPage + 1 }&cntPerPage=${paging.cntPerPage}">Next</a></li>
			  		</ul>
			  		</c:if>	
			</div>
